...
|----|----------|----------|----------|-----------|------|--------|--------------|
| T117 | Azrng.Core 新增 UnauthorizedException（401）并接入异常中间件 | 趁 1.4.0 未发布补齐 401 语义：Azrng.Core 新增 UnauthorizedException（ErrorCode 401，版本 1.19.0→1.20.0）；Azrng.AspNetCore.Core 将 Azrng.Core 引用由 PackageReference 1.8.4 改为 ProjectReference（对齐 Common.EFCore 等主流包），异常中间件新增 UnauthorizedException→401 映射；补两库单测与文档 | 阶段 1 | Claude | DOING | P1 | 2026-07-19 |
