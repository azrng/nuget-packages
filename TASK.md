...
| BL-18c | ALTER 字段结构化 | 字段扩展 | 当前 ALTER 操作通过透传字符串（OptionalSpecifier 等）已能 round-trip（BL-14 已修 14 处缺陷），但 Index/OldIndex/ColumnDropNotNull/ColumnDropDefault/ConvertType/ConstraintState 等结构化字段未建模 | 需结构化访问 ALTER 操作的索引/约束细节时 | 上游 AlterExpression 5.4→HEAD +694 行 + 5 个伴生类，改动量大，round-trip 不受影响 |
...
| BL-19b | KSQL 窗口（ksqlDB） | 方言补全 | 完全缺失。上游 KSQLWindow（6字段+2枚举）+ KSQLJoinWindow（7字段），grammar 约 65 行 | ksqlDB 流式 SQL（`HOPPING`/`TUMBLING`/`SESSION` + `WITHIN`） | Confluent ksqlDB 流处理社区，改动可控（2类+PlainSelect/Join各1字段） |
| BL-19c | CREATE VIEW FORCE/MATERIALIZED/WITH READ ONLY | 方言补全 | grammar createView 缺 FORCE/NO FORCE/SECURE/MATERIALIZED/AUTO REFRESH/WITH READ ONLY；已有 TEMPORARY/RECURSIVE/WITH CHECK OPTION | Oracle 物化视图（`CREATE MATERIALIZED VIEW`）、Exasol AUTO REFRESH、Oracle WITH READ ONLY | 需 grammar 扩展 + CreateView 字段 |
| BL-19d | TableStatement（MySQL 8.2 简写） | 方言补全 | 完全缺失。`TABLE name [ORDER BY] [LIMIT]`（SELECT 简写） | MySQL 8.2+ | 罕见语法 |
| BL-19e | PivotXml（Oracle） | 方言补全 | 完全缺失。`PIVOT XML (...)`（Azrng 仅有 PIVOT 无 XML 变体） | Oracle 旧语法 | 罕见 |
| BL-19f | ParenthesedFromItem（AST 保真） | AST 保真 | grammar 能解析括号 FROM 项，但 visitor unwrap 丢弃括号，round-trip 丢括号 | 需精确保留 FROM 项括号时 | 低频，仅 round-trip 保真场景 |
| BL-19g | ON DUPLICATE KEY UPDATE ... WHERE | 部分缺口 | Insert.DuplicateUpdateSets 支持 SET 子句，缺 WHERE 条件（上游 InsertDuplicateAction.whereExpression） | MySQL 8.0.20+ `ON DUPLICATE KEY UPDATE ... WHERE ...` | 极少用 |
...
- **Azrng**：1217 测试（截至 T092 长期对标缺口修复完成）
...
| T089 | Azrng.JSqlParser STRUCT/ARRAY 复合列类型移植（ARRAY<T> 扁平化 + STRUCT(x INT) 字段进 ArgumentsStringList，含嵌套；全量 1064 测试通过） | DONE | 2026-07-09 |
| T088 | Azrng.JSqlParser BL-06 全量移植 CREATE TABLE 方言与约束结构化（破坏性重构对齐上游 11 类模型，约束结构化 FK/CHECK/EXCLUDE，CTAS/LIKE/RowMovement/Spanner；全量 1052 测试通过） | DONE | 2026-07-09 |
