...
| ID | 任务名称 | 状态 | 更新时间 |
|----|----------|------|----------|
| T114 | Azrng.JSqlParser 非 PG 上游 issue 修复（10 条） | 修复探针复现的 10 条上游 issue：#1169 GROUP BY DESC、#854 INTO @var、#1314 INSERT SET AS 别名、#1589 PRIMARY KEY NONCLUSTERED、#161 OPTION hint、#2298 CAST CHARACTER SET、#2427+#2006 _utf8mb4 introducer、#2428 PROCEDURE ANALYSE、#911 @table 表变量。按方言分 3 批 commit（0aacffc 通用+SQL Server / d65c275 MySQL / 文档收口）。每条改 lexer+grammar+visitor+模型，新增 26 探针 + 28 round-trip 测试，全量 1599→1642 通过。剔除 #2421（BigQuery 小众）。MIGRATION.md 第 14 节 + issue 分类清单状态列同步 | DONE | 2026-07-19 |
