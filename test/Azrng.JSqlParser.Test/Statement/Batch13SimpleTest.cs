...
public class Batch13SimpleTest
{
    ...
    public void Table_FourPartName_RoundTrip()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM s1.db1.sc1.t1");
    ...
    {
        // 3 段不设 ServerName
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM db1.sc1.t1");
        var plainSelect = Assert.IsType<PlainSelect>(stmt);
        var table = Assert.IsType<Azrng.JSqlParser.Schema.Table>(plainSelect.FromItem);
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT SAFE_CAST(x AS INT) FROM t");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT SAFE_CAST(x AS VARCHAR) FROM t");
        Assert.NotNull(stmt);
    ...
    public void NumericBind_RoundTrip()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE id = :1");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE id = :1 AND name = :2");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT FIRST 5 * FROM t");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT SKIP 10 FIRST 5 * FROM t");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t OPTIMIZE FOR 10 ROWS");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t OPTIMIZE FOR 100 ROWS");
        var plainSelect = Assert.IsType<PlainSelect>(stmt);
