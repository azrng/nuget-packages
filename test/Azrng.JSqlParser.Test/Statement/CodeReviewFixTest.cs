...
public class CodeReviewFixTest
{
    ...
    public void H1_Merge_SourceTable_Extracted()
    {
        var tables = new TablesNamesFinder().GetTables(
            CCJSqlParserUtil.Parse("MERGE INTO t USING src ON t.id = src.id WHEN MATCHED THEN DELETE")!);
    ...
    public void H2_DoubleLiteral_InvariantCulture()
    {
        ...
        try
        {
            ...
            Assert.Equal("SELECT 1.5", CCJSqlParserUtil.Parse("SELECT 1.5")!.ToString());
            Assert.Equal("SELECT 1.5, 2.5", CCJSqlParserUtil.Parse("SELECT 1.5, 2.5")!.ToString());
        }
    ...
    {
        var tables = new TablesNamesFinder().GetTables(CCJSqlParserUtil.Parse("TABLE users")!);
    ...
    {
        var tables = new TablesNamesFinder().GetTables(
            CCJSqlParserUtil.Parse("UPDATE a JOIN b ON a.id = b.a_id SET a.x = b.y")!);
    ...
    {
        var tables = new TablesNamesFinder().GetTables(
            CCJSqlParserUtil.Parse("SELECT * FROM users ORDER BY (SELECT name FROM cfg)")!);
        Assert.Contains("users", tables);
    ...
    {
        var tables = new TablesNamesFinder().GetTables(
            CCJSqlParserUtil.Parse("SELECT * FROM users GROUP BY id HAVING COUNT(*) > (SELECT COUNT(*) FROM orders)")!);
    ...
    public void H3_MultipleStatements_Throws()
    {
        Assert.Throws<JSqlParserException>(() =>
            CCJSqlParserUtil.Parse("SELECT 1; DROP TABLE x"));
    }
    ...
    {
        Assert.Null(CCJSqlParserUtil.ParseNullable("SELECT FROM"));
    }
    ...
    public void L2_OffsetRows_RoundTrip()
    {
        Assert.Equal("SELECT * FROM t OFFSET 1 ROWS",
            CCJSqlParserUtil.Parse("SELECT * FROM t OFFSET 1 ROWS")!.ToString());
    }
    ...
    public void L2_OffsetNoRow_RoundTrip()
    {
        Assert.Equal("SELECT * FROM t OFFSET 1",
            CCJSqlParserUtil.Parse("SELECT * FROM t OFFSET 1")!.ToString());
    }
}
