...
public class OverlapsAndMemberOfTest
{
    ...
    public void MemberOf_RoundTrip_ShouldPreserveSemantics()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE 1 MEMBER OF ('[1,2,3]')");
    ...
    public void MemberOf_Not_RoundTrip_ShouldPreserveNot()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE 1 NOT MEMBER OF ('[1,2,3]')");
    ...
    {
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE x NOT MEMBER OF arr");
        var plainSelect = Assert.IsType<PlainSelect>(stmt);
        var where = Assert.IsType<MemberOfExpression>(plainSelect.Where);
    ...
    public void Overlaps_RoundTrip_ShouldPreserveSemantics()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE a OVERLAPS b");
    ...
    public void Overlaps_InComplexWhere_ShouldPreserve()
    {
        ...
        var stmt = CCJSqlParserUtil.Parse("SELECT * FROM t WHERE active = 1 AND a OVERLAPS b");
