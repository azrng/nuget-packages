using Azrng.DistributeLock.Core;
using Azrng.DistributeLock.PostgreSql;
using Dapper;
using Microsoft.Extensions.Options;
using Npgsql;

namespace Azrng.DistributeLock.Pgsql.Test;

/// <summary>
/// 锁丢失通知与安全性测试
/// </summary>
public class LockLostAndSafetyTest
{
    private readonly ILockProvider _lockProvider;
    private readonly DbLockOptions _options;

    public LockLostAndSafetyTest(ILockProvider lockProvider, IOptions<DbLockOptions> options)
    {
        _lockProvider = lockProvider;
        _options = options.Value;
    }

    /// <summary>
    /// 锁行被外部删除后，续期连续失败应触发 LockLostToken 通知
    /// </summary>
    [Fact]
    public async Task LockLostToken_Cancelled_WhenLockRowDeletedExternally()
    {
        var lockKey = Guid.NewGuid().ToString();

        var lockInstance = await _lockProvider.LockAsync(lockKey, TimeSpan.FromSeconds(2));
        Assert.NotNull(lockInstance);

        var lost = new TaskCompletionSource();
        await using var registration = lockInstance!.LockLostToken.Register(() => lost.TrySetResult());

        // 模拟锁行被外部删除（如误删、故障恢复清理）
        await using (var connection = new NpgsqlConnection(_options.ConnectionString))
        {
            await connection.ExecuteAsync(
                $"DELETE FROM {_options.Schema}.{_options.Table} WHERE key = @lockKey", new { lockKey });
        }

        // 续期间隔约666ms，连续失败3次约2秒后应触发通知
        await Task.WhenAny(lost.Task, Task.Delay(TimeSpan.FromSeconds(10)));
        Assert.True(lost.Task.IsCompleted, "锁行被外部删除后应触发锁丢失通知");
        Assert.True(lockInstance.ExtendFailureCount >= 3);

        await lockInstance.DisposeAsync();
    }

    /// <summary>
    /// 已过期的锁不允许续期：外部把 expire_time 改到过去后，续期应失败并触发锁丢失通知
    /// </summary>
    [Fact]
    public async Task Extend_ExpiredLock_Fails_And_NotifiesLost()
    {
        var lockKey = Guid.NewGuid().ToString();

        var lockInstance = await _lockProvider.LockAsync(lockKey, TimeSpan.FromSeconds(2));
        Assert.NotNull(lockInstance);

        var lost = new TaskCompletionSource();
        await using var registration = lockInstance!.LockLostToken.Register(() => lost.TrySetResult());

        // 把过期时间改到过去：若续期 SQL 缺少"仅未过期可续"条件，续期仍会成功并把时间改回未来，不会触发通知
        await using (var connection = new NpgsqlConnection(_options.ConnectionString))
        {
            await connection.ExecuteAsync(
                $"UPDATE {_options.Schema}.{_options.Table} SET expire_time = (now() AT TIME ZONE 'utc') - interval '1 hour' WHERE key = @lockKey",
                new { lockKey });
        }

        await Task.WhenAny(lost.Task, Task.Delay(TimeSpan.FromSeconds(10)));
        Assert.True(lost.Task.IsCompleted, "已过期的锁续期应失败并触发锁丢失通知");

        await lockInstance.DisposeAsync();
    }

    /// <summary>
    /// 锁过期被接管后，旧实例释放不会误删新持有者的锁
    /// </summary>
    [Fact]
    public async Task ExpiredLock_TakenOver_StaleReleaseIsNoop()
    {
        var lockKey = Guid.NewGuid().ToString();

        // 持有者1：1秒过期且不续期，不主动释放
        var lock1 = await _lockProvider.LockAsync(lockKey, TimeSpan.FromSeconds(1), autoExtend: false);
        Assert.NotNull(lock1);

        // 持有者2：等待过期后接管
        var lock2 = await _lockProvider.LockAsync(lockKey, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(5), autoExtend: false);
        Assert.NotNull(lock2);

        // 旧实例此时释放，不应删掉持有者2的锁
        await lock1!.DisposeAsync();
        await using (var connection = new NpgsqlConnection(_options.ConnectionString))
        {
            var count = await connection.ExecuteScalarAsync<long>(
                $"SELECT count(*) FROM {_options.Schema}.{_options.Table} WHERE key = @lockKey", new { lockKey });
            Assert.Equal(1, count);
        }

        var lock3 = await _lockProvider.LockAsync(lockKey, TimeSpan.FromSeconds(5), TimeSpan.FromMilliseconds(500), autoExtend: false);
        Assert.Null(lock3);

        await lock2!.DisposeAsync();
    }
}
