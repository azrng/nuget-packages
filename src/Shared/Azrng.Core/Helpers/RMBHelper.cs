...
public class RmbHelper
{
    ...
    public static string ToRmbUpper(decimal num)
    {
        ...
        //数字的字符串形式  将num乘100并转换成字符串形式 
        var str4 = ((long)(num * 100)).ToString();
        var j = str4.Length; //找出最高位 
        //num的值乘以100的字符串长度 
        if (j > 15) { return "溢出"; }
    ...
    /// <returns></returns> 
    public static string ToRmbUpper(string numStr)
    {
        var valid = decimal.TryParse(numStr, out var num);
        if (!valid)
