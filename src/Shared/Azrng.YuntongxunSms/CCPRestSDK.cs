...
namespace Azrng.YuntongxunSms
{
    ...
    {
        private string _restAddress = null;
        private string _restPort = null;
        private string _mainAccount = null;
        private string _mainToken = null;
        private string _subAccount = null;
        private string _subToken = null;
        private string _voipAccount = null;
        private string _voipPwd = null;
        private string _appId = null;
        private bool _isWriteLog = false;

        //private EBodyType m_bodyType = EBodyType.EType_XML;
        private EBodyType _bodyType = EBodyType.EType_JSON;


        /// <summary>
        /// 服务器api版本
        /// </summary>
        const string _softVer = "2013-12-26";
        ...
        public bool Init(string restAddress, string restPort)
        {
            ...
            if (_restAddress == null || _restAddress.Length < 0 || _restPort == null || _restPort.Length < 0 || Convert.ToInt32(_restPort) < 0)
                return false;
        ...
        {
            _mainAccount = accountSid;
            _mainToken = accountToken;
        }
        ...
        {
            _subAccount = subAccountSid;
            _subToken = subAccountToken;
            _voipAccount = voipAccount;
            _voipPwd = voipPassword;
        }
        ...
        public Dictionary<string, object> QueryAccountInfo()
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/AccountInfo?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
        ...
        public Dictionary<string, object> CreateSubAccount(string friendlyName)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/SubAccounts?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
                ...
                if (_bodyType == EBodyType.EType_XML)
                {
                    ...
                    data.Append("<appId>").Append(_appId).Append("</appId>");
                ...
                else
                {
                    ...
                    data.Append("{");
                    data.Append("\"appId\":\"").Append(_appId).Append("\"");
                ...
                WriteLog("CreateSubAccount requestBody = " + data);
        ...
        public Dictionary<string, object> GetSubAccounts(uint startNo, uint offset)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/GetSubAccounts?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> QuerySubAccount(string friendlyName)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/QuerySubAccountByName?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> SendTemplateSMS(string to, string templateId, string[] data)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/SMS/TemplateSMS?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
                ...
                if (_bodyType == EBodyType.EType_XML)
                {
                    ...
                    bodyData.Append("<appId>").Append(_appId).Append("</appId>");
                ...
                else
                {
                    ...
                    bodyData.Append(",\"appId\":\"").Append(_appId).Append("\"");
        ...
        public Dictionary<string, object> LandingCall(string to, string mediaName, string mediaTxt, string displayNum, string playTimes, int type, string respUrl)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/Calls/LandingCalls?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> VoiceVerify(string to, string verifyCode, string displayNum, string playTimes, string respUrl)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/Calls/VoiceVerify?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> IvrDial(string number, string userdata, string record)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/ivr/dial?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> BillRecords(string date, string keywords)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + mydate);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/BillRecords?sig={4}", _restAddress, _restPort, _softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
                ...
                Encoding myEncoding = Encoding.GetEncoding("utf-8");
                byte[] myByte = myEncoding.GetBytes(_mainAccount + ":" + mydate);
                string authStr = Convert.ToBase64String(myByte);
                request.Headers.Add("Authorization", authStr);
        ...
        private Dictionary<string, object> paramCheckRest()
        {
            ...
            if (_restAddress == null)
            {
                statusCode = 172004;
                statusMsg = "IP空";
            }
            else if (_restPort == null)
            {
                statusCode = 172005;
                statusMsg = "端口错误";
            }
        ...
        private Dictionary<string, object> paramCheckSunAccount()
        {
            ...
            else if (_subToken == null)
            {
                statusCode = 172009;
                statusMsg = "子帐号令牌空";
            }
            else if (_voipAccount == null)
            {
                statusCode = 1720010;
                statusMsg = "VoIP帐号空";
            }
            else if (_voipPwd == null)
            {
                statusCode = 172011;
                statusMsg = "VoIP密码空";
            }
