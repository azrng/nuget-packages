...
| 异常类型 | HTTP 状态码 | 说明 |
|---------|------------|------|
| `UnauthorizedException` | 401 | 未认证（未登录或凭证无效） |
...
// 获取远程 IPv4 / IPv6（直连地址，反代后是网关 IP）
var remoteIp = HttpContext.GetRemoteIpAddressToIPv4();

// 获取客户端真实 IP（优先 X-Forwarded-For / X-Real-IP，推荐配合 UseForwardedHeaders 使用）
var clientIp = HttpContext.GetClientIp();

// 启用转发头处理（可信代理列表可选；仅可信来源的转发头会被消费，防伪造）
app.UseForwardedHeaders("10.0.0.5", "10.0.0.6");
```

**安全边界**: `UseForwardedHeaders` 须在 `UseRouting` 之前注册；未启用该中间件时 `GetClientIp` 的头解析结果可被直连客户端伪造，仅适用于展示、统计等非鉴权场景。
...
| 版本 | 主要变更 |
|------|----------|
| 1.5.0 | **CORS 与预配置 API 精简 + 反向代理 IP 支持 + RequestId 加固（破坏性）**：新增 `UseForwardedHeaders()`/`GetClientIp()` 真实客户端 IP 支持；`RequestIdMiddleware` 入站 ID 白名单校验、无合法头时沿用宿主 `TraceIdentifier`、清理死代码；移除与框架原生 `AddCors`/`AddPolicy` 完全等价的 `AddCorsPolicy()`，高级场景直接使用原生写法；移除 `PreConfigure`/`AddObjectAccessor` 预配置体系（存入的委托从不被 Options 管道消费）；`AddAnyCors`、`AddCorsByOrigins`、`UseCorsPolicy` 保持不变 |
