...
/// <summary>
/// 文档：https://maomi.whuanle.cn/2.design_module.html
/// </summary>
public static class ServiceCollectionExtensions
{
    ...
    public static void AddModule(this IServiceCollection services, Type startupModule)
    {
        ...
        var scope = BuildModule(services, startupModule);
    }
    ...
    {
        // 生成根模块
        var rootTree = new ModuleNode { ModuleType = startupModule, Childs = new HashSet<ModuleNode>() };
        ...
        var moduleAssemblies = new HashSet<Assembly> { startupModule.Assembly };
        var moduleTypes = new HashSet<Type>();
        var injectTypes = new HashSet<Type>();
    ...
    private static void BuildTree(IServiceCollection services, ModuleNode currentNode,
        IEnumerable<InjectModuleAttribute> injectModules)
    {
        services.AddTransient(currentNode.ModuleType);
        if (injectModules == null || !injectModules.Any()) return;
        foreach (var childModule in injectModules)
        {
            var childTree = new ModuleNode { ModuleType = childModule.ModuleType, ParentModule = currentNode };
            ...
            if (isLoop)
            {
                throw new OverflowException(
                    $"检测到循环依赖引用或重复引用！{currentNode.ModuleType.Name} 依赖的 {childModule.ModuleType.Name} 模块在其父模块中出现过！");
            }
    ...
    private static void InitInjectService(IServiceCollection services, Assembly assembly, HashSet<Type> injectTypes)
    {
        ...
        {
            if (item.GetCustomAttributes().FirstOrDefault(x => x.GetType() == typeof(InjectOnAttribute)) is not
                InjectOnAttribute inject)
                continue;

            if (!injectTypes.Add(item))
                continue;
            ...
            if (inject.Scheme == InjectScheme.OnlyInterfaces || inject.Scheme == InjectScheme.Any)
            {
                ...
                {
                    case ServiceLifetime.Transient:
                        interfaces.ToList().ForEach(x => services.AddTransient(x, item));
                        break;
                    case ServiceLifetime.Scoped:
                        interfaces.ToList().ForEach(x => services.AddScoped(x, item));
                        break;
                    case ServiceLifetime.Singleton:
                        interfaces.ToList().ForEach(x => services.AddSingleton(x, item));
                        break;
                }
            }
            ...
            if (inject.Scheme == InjectScheme.OnlyBaseClass || inject.Scheme == InjectScheme.Any)
            {
                ...
                if (baseType == null)
                    throw new ArgumentException($"{item.Name} 注入模式 {nameof(inject.Scheme)} 未找到父类！");
                switch (inject.Lifetime)
                {
                    case ServiceLifetime.Transient:
                        services.AddTransient(baseType, item);
                        break;
                    case ServiceLifetime.Scoped:
                        services.AddScoped(baseType, item);
                        break;
                    case ServiceLifetime.Singleton:
                        services.AddSingleton(baseType, item);
                        break;
                }
            }
            ...
            if (inject.Scheme == InjectScheme.Some)
            {
                ...
                {
                    case ServiceLifetime.Transient:
                        types.ToList().ForEach(x => services.AddTransient(x, item));
                        break;
                    case ServiceLifetime.Scoped:
                        types.ToList().ForEach(x => services.AddScoped(x, item));
                        break;
                    case ServiceLifetime.Singleton:
                        types.ToList().ForEach(x => services.AddSingleton(x, item));
                        break;
                }
            }
        }
