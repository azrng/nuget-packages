...
- `SELECT` — DISTINCT/ALL、TOP（PERCENT/WITH TIES）、JOIN（INNER/LEFT/RIGHT/FULL/CROSS/NATURAL/SEMI/ANTI #2643、`ARRAY JOIN` #2482）、CTE（WITH RECURSIVE，支持 DML，递归 `SEARCH`/`CYCLE` 子句 #2566）、UNION/INTERSECT/EXCEPT、子查询、GROUP BY、HAVING、WINDOW、`MATCH_RECOGNIZE` 行模式识别（#2634）、ClickHouse `ORDER BY ... WITH FILL`/`INTERPOLATE`（#2469）、`COLUMNS(...) APPLY/EXCEPT/REPLACE`（#2631）、PREFERRING（Exasol Skyline）、ORDER BY、LIMIT/OFFSET、FETCH、FOR UPDATE/SHARE（OF 多表、WAIT/NOWAIT/SKIP LOCKED）、`OVERLAPS`、`MEMBER OF`、三元条件 `a ? b : c`（#2436）
...
- PG DDL：`CREATE ROLE/USER/GROUP`（#2555）、`CREATE DOMAIN`（#2536）、`CREATE EXTENSION`（#2553）、`CREATE PUBLICATION/SUBSCRIPTION`（#2554）、`DO $$ ... $$` 匿名块（#2589）、`COMMENT ON` 多目标（#2639）
- MySQL DDL：`CREATE TRIGGER`（#2548）、`CREATE EVENT`（#2547）
- DuckDB：`COPY`/`ATTACH`/`PRAGMA`/`CREATE MACRO`（#2643）
- BigQuery：`UNNEST [WITH OFFSET]`、`EXPORT DATA`、`LOAD DATA`、`ASSERT`（#2642）
