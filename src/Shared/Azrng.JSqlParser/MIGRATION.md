...
| 批次 | 内容 | 风险 | 对照锚点 | 状态 |
|------|------|------|----------|------|
| **批 1** | 删 visitor 接口 default method（下沉到 Adapter）；删无 context 重载 | 低 | `[形]` | ✅ 已完成（动作 B 下沉完成；**动作 A 经评估保留**：`ISelectVisitor` 的 6 个无 context 便利重载是 `void` 返回的有意便利设计，对"只遍历不取返回值"的调用方有意义，不是 Java 风格遗留，保留并在 `ValuesSelectVisitorTest` 中作为公开 API 契约覆盖） |
...
| **批 7** | 枚举 SCREAMING_CASE → PascalCase（`ForMode`/`AlterOperation`/`ReturningReferenceType`/`DateTimeType`） | **高** | `[锚][风险]` 公开 API 破坏性，建议 2.0 | ✅ 已完成（4 枚举共 60 值全改 PascalCase：ForMode 6 值 / AlterOperation 47 值 / ReturningReferenceType 2 值 / DateTimeType 5 值；全部引用点同步；`AlterExpression.ToString()` 与 `DateTimeLiteralExpression.ToString()` 加 `ToUpperInvariant()` 保证 SQL 输出不变；详见第十六章 2026-07-18 记录） |
...
| 日期 | 批次 | 改动摘要 | 影响面 | 验证 |
|------|------|----------|--------|------|
| 2026-07-17 | 批 1（动作 B） | `ExpressionVisitor<T>` 接口的 17 个 default method（含递归逻辑）下沉为纯签名声明；递归实现统一搬到 `ExpressionVisitorAdapter<T>`（补全原先缺失的 12 个边缘节点 override：TrimFunction/CollateExpression/TimezoneExpression/ArrayConstructor/ArrayExpression/RowConstructor/NextValExpression/AnyComparisonExpression/DateUnitExpression/OracleHint/OracleNamedFunctionParameter/PostgresNamedFunctionParameter）；`TablesNamesFinder`（直接实现接口）补全对应 17 个 Visit 方法 + 加 `using Expression.Cnf`。**动作 A（删无 context 重载）经评估保留**：`ISelectVisitor` 的 6 个无 context 便利重载（`void Visit(PlainSelect)` 等）是 `void` 返回的有意便利设计，对"只遍历不取返回值"的调用方有意义，不是 Java 风格遗留；`ValuesSelectVisitorTest.SelectVisitor_ConvenienceOverload_VisitValues_Works` 将其作为公开 API 契约覆盖 | 库内 3 文件；公开接口行为不变 | 全量 1431 项通过，0 失败 |
...
| 2026-07-17 | 批 2b | `FromItem` 接口契约现代化：`GetAlias()/SetAlias()` → `Alias? Alias { get; set; }` 属性（6 个实现类 Table/JsonTable/LateralView/ParenthesedSelect/TableFunction/Values 此前均已有该属性，getter/setter 是纯冗余转发）；6 个实现类的 `GetAlias()/SetAlias()` 保留并标 `[Obsolete]` 转发以向后兼容；`AstBuilderVisitor` 内部 `SetAlias()` 调用改为属性赋值；测试 `ValuesTest` 改用属性 | 接口签名变更（[锚]），6 实现类 + visitor + 测试；旧方法保留 Obsolete 转发，无破坏性 | 全量 1431 项通过，0 失败 |
| 2026-07-17 | 批 3 | 6 个字面量叶子类加 `sealed`（NullValue/LongValue/DoubleValue/StringValue/HexValue/BooleanValue，无子类）；`Alias` 加 `sealed` + 值相等 `Equals/GetHashCode`（按 Name+UseAs，使 LINQ Distinct/Contains 行为符合 SQL 别名语义）；补 2 项 Alias 值相等测试。**运算符薄壳类（24 个）暂未加 sealed**：收益低（无人继承也不会继承），收敛批 3 范围避免过度设计；**Column/Table Equals 暂不加**：字段多且含 AST 引用，相等语义复杂，留待有 Distinct 需求时再加 | 库内 7 文件 + 测试 1 文件；无 API 破坏（sealed 只禁止继承，无子类） | 全量 1433 项通过（净增 2），0 失败 |
| 2026-07-17 | 批 4 | `BinaryExpression.GetStringExpression()` 抽象方法 → `OperatorSymbol` 公开只读属性（28 个运算符子类同步改 override：13 算术 + 15 关系）；`StringExpression` 计算属性 → `OperatorExpressionText`；`GetStringExpression()` 保留为 `[Obsolete]` 转发到 `OperatorSymbol`（向后兼容）；`WhereConditionsExtractor` 与测试改用 `OperatorSymbol`。**注意**：原计划 `protected abstract`，实际改为 `public abstract`——因为运算符符号对外有意义（WhereCondition.Operator 取它），外部类需访问 | 库内 30 文件 + 测试 1 文件；旧方法保留 Obsolete 转发，无破坏性 | 全量 1433 项通过，0 失败 |
...
| 2026-07-18 | 文档同步（T110 收口） | 本次盘点发现 MIGRATION.md 第十五章状态表与实际代码不一致，做文档纠错（**零代码改动**）：① **批 7 状态行**：第十五章原写"⏸ 留 2.0"，但代码早已全 PascalCase（`ForMode.Update`/`AlterOperation.Add` 等无任何 SCREAMING_CASE 残留），第十六章也有 2026-07-18 完整改造记录 → 改为"✅ 已完成"；② **批 1 动作 A 定性**：原写"挪至 2.0"（理由是 `ValuesSelectVisitorTest` 依赖）→ 经评估，`ISelectVisitor` 的 6 个无 context 便利重载是 `void` 返回的有意便利设计，对"只遍历不取返回值"的调用方有意义，**不是 Java 风格遗留**，改为"经评估保留"，测试作为公开 API 契约覆盖；③ **批 9 记录措辞**：批 9 末尾"Expression/Statement 接口暂不改...留 2.0" → 改为指向批 9b"已由人工完成"。至此 T110 全部 10 批代码 + 文档彻底对齐 | 纯文档，MIGRATION.md 4 处；无代码改动 | 全量 1436 项通过（未重跑，代码无改动） |
