...
public partial class AstBuilderVisitor
{
    ...
    public override object VisitPlainSelect(JSqlParserGrammar.PlainSelectContext context)
    {
        ...
        {
            select.OptionHints = string.Join(", ",
                optionCtx.optionHint().Select(h => GetOriginalText(h).Trim()).Where(s => !string.IsNullOrEmpty(s)));
        }
    ...
    private GroupByElement BuildGroupByElement(JSqlParserGrammar.GroupByClauseContext ctx)
    {
        ...
        if (columns != null && columns.Length > 0)
        {
            ...
            {
                gb.GroupByExpressions = columns.Select(c => (Expression.IExpression)Visit(c.expression())).ToList();
            }
        }
        return gb;
    }
