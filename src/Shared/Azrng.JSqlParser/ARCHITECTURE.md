...
| `stmt.ExtractTableNames()` | `new TablesNamesFinder().GetTables(stmt)` | 提取表名，返回只读集合 |
...
| 命名空间 | 用途 |
|----------|------|
| `Azrng.JSqlParser` | C# 风格遍历扩展方法（`ExpressionExtension`/`StatementExtension`：Descendants/Walk/ExtractTableNames） |
