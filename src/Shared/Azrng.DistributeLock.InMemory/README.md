...
内存分布式锁基于 `ConcurrentDictionary` 实现，适用于单进程内的同步。当多个线程尝试获取同一个锁时：

1. 第一个请求会成功获取锁，键值对（锁值 + 过期时间）会被添加到字典中
2. 后续请求会进入等待状态，直到锁被释放、过期或等待超时
3. 锁会在 using 语句结束时自动释放，从字典中移除对应键值对
4. 锁支持过期：持有者未释放且未续期时，过期后可被其他请求接管；释放和续期都会校验锁值，不会误操作他人持有的锁（与 Redis / PostgreSQL 实现语义一致）
...
* 0.4.0
  * 实现真实的过期语义：锁过期后可被接管，续期和释放校验锁值，与 Redis / PostgreSQL 行为对齐
  * 默认过期时间由 30 秒调整为 5 秒，与接口文档及其他实现保持一致
  * 跟随 Core 0.4.0：`LockAsync` 返回 `LockInstance?`，支持 `LockLostToken`
* 0.3.0
  * 更新延期代码
* 0.2.0
  * 更新README.md
