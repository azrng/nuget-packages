...
namespace Common.HttpClients
{
    ...
    public interface IHttpHelper
    {
        ...
        Task<IHttpResult<Stream>> GetStreamAsync(string url, HttpSendOptions? opt = null,
                                                 CancellationToken cancellation = default);

        /// <summary>
        /// GET请求返回自定义内容（T 为 string 时返回原始响应体）
        /// </summary>
        ...
        Task<IHttpResult<T>> GetAsync<T>(string url, HttpSendOptions? opt = null, CancellationToken cancellation = default);

        /// <summary>
        /// POST请求返回自定义内容（data 为字符串时按原始 JSON 发送；T 为 string 时返回原始响应体）
        /// </summary>
        ...
        /// <param name="cancellation"></param>
        /// <returns>包含反序列化对象的HttpResult</returns>
        Task<IHttpResult<T>> PutAsync<T>(string url, object data, HttpSendOptions? opt = null,
                                         CancellationToken cancellation = default);

        /// <summary>
        /// Delete请求返回自定义内容（T 为 string 时返回原始响应体）
        /// </summary>
        /// <typeparam name="T">响应内容</typeparam>
        /// <param name="url">请求地址</param>
        /// <param name="opt">请求选项（查询参数、请求头）</param>
        ...
        /// <param name="cancellation"></param>
        /// <returns>包含反序列化对象的HttpResult</returns>
        Task<IHttpResult<T>> PatchAsync<T>(string url, object data, HttpSendOptions? opt = null,
                                           CancellationToken cancellation = default);

        /// <summary>
        /// 底层逃生舱口，直接发送原始 HttpRequestMessage，返回原始 HttpResponseMessage（调用方自行处理响应与状态码）
        /// </summary>
        ...
        Task<IHttpResult<DownloadResult>> DownloadFileAsync(string url, string filePath, HttpSendOptions? opt = null,
                                                            CancellationToken cancellation = default);
    }
}
