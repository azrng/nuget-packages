...
public static class DeepCopyByExpressionTrees
{
    ...
    private static readonly MethodInfo SetValueMethod =
        FieldInfoType.GetMethod(nameof(Array.SetValue), new Type[]
                                                        {
                                                            ObjectType,
                                                            ObjectType
                                                        })!;
    ...
    private static readonly MethodInfo DeepCopyByExpressionTreeObjMethod =
        typeof(DeepCopyByExpressionTrees).GetMethod(nameof(DeepCopyObj), BindingFlags.NonPublic | BindingFlags.Static)!;
    ...
    public static T? DeepCopy<T>(this T original, Dictionary<object, object>? copiedReferencesDict = null) =>
        (T?)DeepCopyObj(original, false,
            copiedReferencesDict ?? new Dictionary<object, object>());

    private static object? DeepCopyObj(object? original, bool forceDeepCopy,
        ...
        CreateCompiledLambdaCopyFunctionForType(Type type)
    {
        ...
        FieldsCopyExpressions(type, inputParameter, inputDictionary, outputVariable, boxingVariable,
            expressions);
        ...
        var lambda = CombineAllIntoLambdaFunctionExpression(inputParameter, inputDictionary, outputVariable, endLabel,
    ...
    {
        var ifNullThenReturnNullExpression = Expression.IfThen(Expression.Equal(inputParameter, Expression.Constant(null, ObjectType)),
                                                                ...
                                                                List<Expression> expressions)
    {
        ...
        var storeReferencesExpression =
            Expression.Assign(Expression.Property(inputDictionary, ObjectDictionaryType.GetProperty("Item")!, inputParameter),
                                                      ...
                                                      List<Expression> expressions)
    {
        ...
        var assignExpression = ArrayFieldToArrayFieldAssignExpression(inputParameter, inputDictionary, outputVariable,
            elementType, type, indices);
    ...
    private static BinaryExpression ArrayFieldToArrayFieldAssignExpression(ParameterExpression inputParameter,
                                                                           ParameterExpression inputDictionary,
                                                                           ParameterExpression outputVariable, Type elementType,
                                                                           Type arrayType,
                                                          ...
                                                          ParameterExpression indexVariable, Expression loopToEncapsulate, int dimension)
    {
        ...
        var newLoop = Expression.Loop(Expression.Block([], Expression.IfThen(Expression.GreaterThanOrEqual(indexVariable, lengthVariable),
                    Expression.Break(endLabelForThisLoop)), loopToEncapsulate,
                                                      ...
                                                      List<Expression> expressions)
    {
        ...
        var fieldToNullExpression = Expression.Call(Expression.Constant(field), SetValueMethod, boxingVariable,
    ...
    private static void ReadonlyFieldCopyExpression(Type type, FieldInfo field, ParameterExpression inputParameter,
                                                    ParameterExpression inputDictionary, ParameterExpression boxingVariable,
                                                    List<Expression> expressions)
    {
        ...
        var fieldDeepCopyExpression = Expression.Call(Expression.Constant(field, FieldInfoType), SetValueMethod,
            boxingVariable,
