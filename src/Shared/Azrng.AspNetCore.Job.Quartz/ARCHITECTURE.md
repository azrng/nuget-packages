...
public IJob NewJob(TriggerFiredBundle bundle, IScheduler scheduler)
{
    ...
    // 以 job 实例为键存储 scope，ReturnJob 时据此清理
    _scopes[job] = scope;
    return job;
}
...
{
    // 作业执行完毕立即释放对应 scope，避免内存泄漏
    if (_scopes.TryRemove(job, out var scope)) scope.Dispose();
}
```
...
- 支持 Scoped 服务注入到作业中
- 以 `IJob` 实例为键跟踪 Scope，`ReturnJob` 在作业执行完毕后立即 Dispose 对应 Scope（修复原先 Scope 永不释放的泄漏）
- 调度器关闭时由 `DisposeAllScopes` 兜底清理尚未回收的 Scope
...
作业执行历史是一套独立子系统，贯穿「监听写入 → 内存存储 → 周期清理 → 统计查询」四个环节，用于在无需外部数据库的前提下提供作业可观测性，同时避免内存无界增长。
...
QuartzJobListener.JobWasExecuted()
    │  组装 JobExecutionHistory（成功/失败/中断结果、耗时、JobDataMap）
    │  受 EnableJobHistory 开关控制
    ▼
IJobExecutionHistoryService.AddHistoryAsync()   ← InMemoryJobExecutionHistoryService（ConcurrentDictionary，以 ExecutionId 为键）
    │
    │  ……历史记录随作业执行不断累积……
    ▼
JobHistoryCleanupHostedService（Timer 周期触发）
    │  按 JobHistoryRetentionDays 计算截止时间
    ▼
IJobExecutionHistoryService.CleanupExpiredHistoryAsync()   ← 删除 StartTime 早于截止时间的记录
```

#### 写入侧：QuartzJobListener

**职责**：在 `JobWasExecuted` 回调中组装并写入历史记录。

**核心代码**：[QuartzJobListener.cs:58](Listeners/QuartzJobListener.cs#L58-L122)

**写入内容**（`JobExecutionHistory`）：
- `ExecutionId` - 本次执行的唯一 ID（`Guid`）
- `JobName` / `JobGroup` - 作业标识
- `TriggerName` / `TriggerGroup` - 触发器标识
- `StartTime` / `EndTime` - 开始与完成时间（本地时间）
- `Result` - 执行结果（`Success` / `Failed` / `Interrupted`，由是否抛出 `JobExecutionException` 判定）
- `ErrorMessage` / `ExceptionStackTrace` - 异常信息（仅失败时）
- `JobData` - 合并后的 `JobDataMap`

**写入开关**：
- `EnableJobHistory = true`（默认）且 `IJobExecutionHistoryService` 已注册时才写入；写入失败仅记录错误日志，不影响作业执行结果。

#### 存储侧：InMemoryJobExecutionHistoryService

**职责**：基于 `ConcurrentDictionary<string, JobExecutionHistory>` 的内存实现，以 `ExecutionId` 为键。

**核心代码**：[InMemoryJobExecutionHistoryService.cs:11](Services/InMemoryJobExecutionHistoryService.cs#L11-L110)

**注意**：`AddHistoryAsync` 使用 `TryAdd`，若 `ExecutionId` 重复（概率极低，因使用 `Guid`）会静默丢弃后一条；这是内存实现的已知行为，如需更严格的去重或并发语义，可替换为自定义实现。

#### 清理侧：JobHistoryCleanupHostedService

**职责**：后台托管服务，周期清理过期历史记录，避免内存无界增长。

**核心代码**：[JobHistoryCleanupHostedService.cs:12](Schedules/JobHistoryCleanupHostedService.cs#L12-L63)

**清理策略**：
- 触发方式：`System.Threading.Timer`，非 Quartz 触发器
- 首次延迟：**1 小时**（`InitialDelay`）
- 清理周期：**每 24 小时**（`Period`）
- 保留天数：`QuartzOptions.JobHistoryRetentionDays`，默认 **30 天**
- 判定依据：`StartTime < DateTime.Now.AddDays(-retentionDays)` 的记录视为过期并删除
- 清理异常被捕获并记录错误日志，不会导致服务停止

**注册方式**：由 `AddQuartzService()` 自动注册为 Hosted Service，无需手动配置：
...
除自动周期清理外，`IJobExecutionHistoryService` 还提供：

| 方法 | 说明 |
|------|------|
| `CleanupExpiredHistoryAsync(retentionDays)` | 手动触发一次清理，可指定保留天数 |
| `GetHistoryAsync(jobName, jobGroup, pageIndex, pageSize)` | 分页查询指定作业的历史记录 |
| `GetRecentHistoryAsync(count)` | 获取最近 N 条历史（默认 50） |
| `GetStatisticsAsync(jobName, jobGroup)` | 获取作业执行统计（总次数、成功/失败次数、平均耗时、最后执行时间与结果） |

统计结果（`JobExecutionStatistics`）包含计算字段 `SuccessRate`（成功率百分比）。

#### 相关配置项（QuartzOptions）

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `EnableJobHistory` | `true` | 是否写入作业执行历史 |
| `EnableDetailedLogging` | `true` | 是否记录详细的作业执行日志（开始/成功日志） |
| `JobHistoryRetentionDays` | `30` | 历史记录保留天数，超期由清理服务删除 |

#### 扩展点：自定义持久化存储

默认内存实现重启后丢失。若需持久化到数据库，实现 `IJobExecutionHistoryService` 并替换注册即可，清理服务与监听器无需改动：
...
> 注：内置的 `QuartzJobListener` 已由 `JobHostedService` 在调度器启动时自动注册到 `ListenerManager`（用于记录执行日志与历史），上面的自定义监听器属于在此基础上的额外扩展。
...
- 历史记录的自动清理、手动清理与统计查询详见 [核心组件 §8 作业执行历史与自动清理](#8-作业执行历史与自动清理)
- 默认按 `JobHistoryRetentionDays`（30 天）周期清理，无需手动调用

### 程序集扫描

- 默认仅扫描入口程序集和调用程序集，范围可控
- 作业分布在多个程序集时，使用 `AssemblyNamesToScan` 显式指定
