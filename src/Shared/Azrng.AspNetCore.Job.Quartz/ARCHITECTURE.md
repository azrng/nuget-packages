...
- 历史记录由 `JobHistoryCleanupHostedService` 按 `JobHistoryRetentionDays`（默认 30 天）自动周期清理（首次延迟 1 小时，之后每 24 小时一次），无需手动调用
- 如需手动清理：`await _historyService.CleanupExpiredHistoryAsync(retentionDays: 30);`
